...
dependency "nsg" {
  config_path = find_in_parent_folders("nsg-internal")
  // config_path = "../virtual-network"
...
locals {
  ...
  address_prefixes = local.layer_vars.locals.address_prefixes["internal"]
}
...
inputs = merge(
  local.shared_dependency_vars.inputs,
  {
    ...
    name = lower(join("", ["sneti-", local.shared_dependency_vars.inputs.name]))
