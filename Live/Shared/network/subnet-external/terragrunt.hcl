...
dependency "nsg" {
  // config_path = find_in_parent_folders("nsg-external")
  config_path = "../nsg-external"
...
locals {
  ...
  address_prefixes = local.layer_vars.locals.address_prefixes["external"]
}
...
inputs = merge(
  local.shared_dependency_vars.inputs,
  {
    ...
    name = lower(join("", ["snete-", local.shared_dependency_vars.inputs.name]))
