...
locals {
  ...
  address_prefixes = local.layer_vars.locals.address_prefixes["bastion"]
}
...
inputs = merge(
  local.shared_dependency_vars.inputs,
  {
    ...
    name = lower(join("", ["snbast-", local.shared_dependency_vars.inputs.name]))
