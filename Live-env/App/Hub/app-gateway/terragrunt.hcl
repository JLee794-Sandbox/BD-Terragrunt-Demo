...
dependency "hub-vnet" {
  config_path = find_in_parent_folders("Network/Hub/virtual-network")
...
dependency "nsg-waf" {
  config_path = find_in_parent_folders("Network/Hub/nsg-waf")
...
dependency "subnet-waf" {
  config_path = find_in_parent_folders("Network/Hub/subnet-waf")
    ...
    backend_address_pool = [
      {
        ...
        name  = "${lower(join("", ["agw-", module.azure_naming.name]))}-be-pool"
      }
    ]
