...
locals {
  ...
  address_prefixes       = local.layer_vars.locals.address_prefixes["aks-ilb"]
}
...
inputs = merge(
  local.shared_dependency_vars.inputs,
  {
    ...
    name                      = lower(join("", ["snetaksilb-", local.shared_dependency_vars.inputs.name]))
